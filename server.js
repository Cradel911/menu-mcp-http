...
app.post("/messages", auth, express.json({ limit: "2mb" }), async (req, res) => {
  try {
    // Different MCP SDK versions expose different handler names.
    ...
    return res.status(500).json({
      error:
        "SSEServerTransport handler not found. Check your @modelcontextprotocol/sdk version and update the /messages handler accordingly.",
    });
  } catch (e) {
